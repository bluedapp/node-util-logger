...
export interface Loggers { accessLog: (res: any) => any, errorLog: (res: any) => any, warnLog: (res: any) => any }
...
export interface LoggerClient {
  ...
  warn: (error: Error, data?: Record<string, any>) => void
}
...
export default abstract class Logger implements LoggerIntl {
  ...
  getLogger (logType: string) {
    ...
    return {
      ...
      warn (error: Error, data?: Record<string, any>) {
        const now = Date.now()
        const datetime = formatTime(now)
        const err = {
          err_msg: error.message,
        ...
        // 添加本地环境的colors输出
        if (isLocal) {
          console.warn(colors.yellow(results))
        }
        logger.warnLog(results)
      },
    }
  }
